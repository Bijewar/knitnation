...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    increaseQuantity(state, action) {
      ...
      const item = state.items.find(item => item.id === id);
      if (item) {
        item.quantity = Math.min(item.quantity + 1, 10); // Adjust max limit as needed
      }
    },
    decreaseQuantity(state, action) {
      ...
      const item = state.items.find(item => item.id === id);
      if (item) {
        item.quantity = Math.max(item.quantity - 1, 1); // Adjust min limit as needed
      }
    },
...
export const { addToCart, updateQuantity,  setAuthenticated, // Make sure this is exported
  removeFromCart, clearCart,decreaseQuantity,increaseQuantity } = cartSlice.actions;
